<template>
    ...
    <view class="search-box">
      <uni-search-bar class="search-item" @input="input" :bgColor="'#fff'" :radius="100" :cancelButton="none" @cancel="add"></uni-search-bar>
    </view>
    <!-- 搜索历史 -->
    <view class="history-box" v-if="msg ? false:true">
      <!-- 搜索标题 -->
      <view class="history-title">
        <text>搜索历史</text>
       <uni-icons type="trash" size="17" @click="deleteMsg"></uni-icons>
      </view>
      <!-- 搜索列表 -->
      <view class="history-list">
       <uni-tag class="icon-box" :text="item" v-for="(item,i) in list" :key="i" size="small" @click="add(item)"></uni-tag>
      </view>
    </view>
    <!-- 搜索列表区域 -->
    <view class="search-list" v-for="(item,i) in searchList" :key="i" @click="gotolist(item.goods_id)">
